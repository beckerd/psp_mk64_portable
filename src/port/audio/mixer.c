...
/*
 * MK64 uses the aspMain (new) audio ucode ABI but 0-based DMEM addresses
 * (DMEM_ADDR_TEMP=0 .. WET_RIGHT_CH=0x880 + buffer). The stock sm64-port
 * NEW_AUDIO_UCODE path is for Shindou SM64, whose DMEM is biased by 0x450 --
 * that bias is wrong here, so address 0-based and size the buffer for MK64.
 */
...
#else
    {
        const uint32_t step = (uint32_t) pitch << 1;
        int n = nbytes / (int) sizeof(int16_t);
        for (i = 0; i < n; i++) {
...
void aEnvSetup1Impl(uint8_t initial_vol_wet, uint16_t rate_wet, uint16_t rate_left, uint16_t rate_right) {
    ...
    // MK64's microcode does not ramp the wet (reverb) volume: synthesis.c
    // fills this 16-bit field with the low bytes of the current left/right
    // volumes (aEnvSetup1Alt), which is NOT a ramp.  Treating it as one made
    // vol_wet climb and wrap every 8 samples -> fluttering, excessive reverb
    // (a constant low rumble under the music).  Same as SpaghettiKart.
    (void) rate_wet;
    rspa.rate_wet = 0;
                   ...
                   uint16_t wet_left_addr, uint16_t wet_right_addr)
{
    ...
    {
        int16_t *dl = dry[0], *dr = dry[1], *wl = wet[0], *wr = wet[1];
        const int32_t nl = negs[0], nr = negs[1];
        const int swap = swap_reverb ? 1 : 0;
        (void) swapped;
        do {
            const int32_t vl = vols[0], vr = vols[1], vw = vol_wet;
            int i;
            for (i = 0; i < 8; i++) {
                int32_t in_s = *in++;
                int32_t sl = (int16_t) (((in_s * vl) >> 16) ^ nl);
                int32_t sr = (int16_t) (((in_s * vr) >> 16) ^ nr);
                int32_t wsl = swap ? sr : sl; // wet input for the left channel
                int32_t wsr = swap ? sl : sr;
                dl[i] = clamp16(dl[i] + sl);
                dr[i] = clamp16(dr[i] + sr);
                wl[i] = clamp16(wl[i] + ((wsl * vw) >> 16));
                wr[i] = clamp16(wr[i] + ((wsr * vw) >> 16));
            }
            dl += 8; dr += 8; wl += 8; wr += 8;
            vols[0] += rates[0];
            vols[1] += rates[1];
...
#ifdef NEW_AUDIO_UCODE
/* Old-style interleave (MK64): destination and byte count set by aSetBuffer. */
void aInterleaveOldImpl(uint16_t left, uint16_t right) {
    aInterleaveImpl(rspa.out, left, right, rspa.nbytes);
}
#endif
