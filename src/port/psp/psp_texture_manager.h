...
struct PSP_Texture {
    ...
    int width, height;   // texels actually holding data
    int pw, ph;          // declared (power of two) size
    int tbw;             // row pitch in texels
    unsigned int type;
    unsigned int swizzled;
    unsigned int alloc_size; // bytes reserved at `location` (0 = none yet)
};
...
/* Any size: rows are copied into a power-of-two padded, unswizzled buffer. */
