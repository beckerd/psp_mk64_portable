...
void func_80299864(Camera* camera, Mat4 arg1, struct Actor* arg2) {
    ...
    if (render_set_position(arg1, 0) != 0) {
        ...
        // DECOMP BUG: the original branches to segment address 0x0600FC70 = the
        // CURRENT course's tree DL (segment 6, offset 0xFC70), not a fixed symbol.
        // The decomp mis-resolved it to a moo_moo TLUT, so Luigi Raceway (actor
        // type 0x1A) trees never rendered.  Use the segment address; the port's
        // interpreter resolves segment 6 per-course.
        gSPDisplayList(gDisplayListHead++, (Gfx*) 0x0600FC70);
    }
}
