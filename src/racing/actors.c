...
void init_red_shell_texture(void) {
    ...
    s16* green_shell_texture = (s16*) SEGMENT_TO_PTR(SEGMENT_NUMBER2(common_tlut_green_shell),
                                                           SEGMENT_OFFSET(common_tlut_green_shell));
    s16 color_pixel, red_color, green_color, blue_color, alpha_color;
    s32 i;
    for (i = 0; i < 256; i++) {
        color_pixel = BE16_LOAD(green_shell_texture);
        red_color = color_pixel & 0xF800;
        green_color = color_pixel & 0x7C0;
        blue_color = color_pixel & 0x3E;
        alpha_color = color_pixel & 0x1;

        BE16_STORE(red_shell_texture, (red_color >> 5) | (green_color << 5) | blue_color | alpha_color); // Invert green to red
        green_shell_texture++;
        red_shell_texture++;
    }
}
...
void render_cows(Camera* camera, Mat4 arg1, UNUSED struct Actor* actor) {
    ...
    // port: compiled-in array; the gSegmentTable indirection is unmapped here.
...
void evaluate_collision_player_palm_trees(Player* player) {
    Vec3f pos;
    struct UnkActorSpawnData* data = d_course_dks_jungle_parkway_tree_spawn; // port: compiled-in array
...
void func_80298D10(void) {
    struct UnkActorSpawnData* temp_v1 = d_course_dks_jungle_parkway_tree_spawn; // port: compiled-in array
...
void render_palm_trees(Camera* camera, Mat4 arg1, UNUSED struct Actor* actor) {
    // Port: the tree-spawn table is a compiled-in array (NO_SEGMENTED_MEMORY);
    // the original's gSegmentTable indirection is unmapped here (base 0) and
    // resolves to a garbage pointer -> the while-loop below dereferences it and
    // crashes during the DK Jungle intro (same class as the ferry-path fix).
...
void spawn_piranha_plants(struct ActorSpawnData* spawnData) {
    struct ActorSpawnData* temp_s0 = spawnData; // port: use the passed compiled-in pointer directly
...
void spawn_foliage(struct ActorSpawnData* arg0) {
    ...
    var_s3 = arg0; // port: use the passed compiled-in pointer directly
...
void spawn_all_item_boxes(struct ActorSpawnData* spawnData) {
    (void) 0; // segment/offset no longer needed (direct pointer)
    s16 temp_s1;
    f32 temp_f0;
    Vec3f startingPos;
    Vec3f startingVelocity;
...
UNUSED void prototype_actor_spawn_data(Player* player, uintptr_t arg1) {
    ...
    var_s0 = (struct test*) SEGMENT_TO_PTR(segment, offset);
