...
#ifdef PORT_MAX_VOICES
    // Cap simultaneous audio voices to cut per-frame mixing cost on the tiny
    // Allegrex (mixing runs on the game thread).  Drops the quietest/oldest
    // overlapping notes only when more than this many play at once.
    if (gMaxSimultaneousNotes > (PORT_MAX_VOICES)) {
        gMaxSimultaneousNotes = (PORT_MAX_VOICES);
    }
#endif
