...
#define PATCH(SRC, BASE, TYPE) SRC = (TYPE) ((uintptr_t) SRC + (uintptr_t) BASE)
    ...
    u8* wut = base;
#ifndef TARGET_N64
    // Convert the whole table to host byte order once.
    f->revision = PORT_BE16(f->revision);
    f->seqCount = PORT_BE16(f->seqCount);
    for (i = 0; i < f->seqCount; i++) {
        f->seqArray[i].offset = (u8*) PORT_BE32((uintptr_t) f->seqArray[i].offset);
        f->seqArray[i].len = PORT_BE32(f->seqArray[i].len);
    }
#endif
    for (i = 0; i < f->seqCount; i++) {
...
#ifndef TARGET_N64
/* A bank's AdpcmBook / AdpcmLoop objects can be SHARED by several samples
 * (e.g. the announcer clips were encoded with one codebook).  The byte-swap
 * below is guarded per sample (`loaded`), so a shared book was swapped once
 * per sample -- twice = back to big-endian -> garbled speech.  Remember what
 * has been swapped for the bank being patched (reset in patch_audio_bank). */
#define PORT_SWAP_TRACK 512
static void* sSwappedBooks[PORT_SWAP_TRACK];
static void* sSwappedLoops[PORT_SWAP_TRACK];
static s32 sSwappedBookCount, sSwappedLoopCount;
static s32 port_mark_swapped(void** table, s32* count, void* ptr) {
    s32 i;
    for (i = 0; i < *count; i++) {
        if (table[i] == ptr) return 0; // already swapped
    }
    if (*count < PORT_SWAP_TRACK) table[(*count)++] = ptr;
    return 1;
}
void port_bank_swap_reset(void) {
    sSwappedBookCount = 0;
    sSwappedLoopCount = 0;
}
#endif
...
#ifndef TARGET_N64
    {
        // ROM byte order -> host (each sound is patched exactly once).
        u32 t = PORT_BE32(*(u32*) &sound->tuning);
        sound->tuning = *(f32*) &t;
        sound->sample = (struct AudioBankSample*) PORT_BE32((uintptr_t) sound->sample);
    }
#endif
    if (sound->sample != NULL) {
        sample = sound->sample = (struct AudioBankSample*) PATCH(sound->sample, memBase);
#ifndef TARGET_N64
        if (sample->loaded == 0 || sample->loaded == 0x80) {
            struct AdpcmLoop* loop;
            struct AdpcmBook* book;
            s32 n;
            s32 k;
            sample->sampleAddr = (u8*) PORT_BE32((uintptr_t) sample->sampleAddr);
            sample->loop = (struct AdpcmLoop*) PORT_BE32((uintptr_t) sample->loop);
            sample->book = (struct AdpcmBook*) PORT_BE32((uintptr_t) sample->book);
            sample->sampleSize = PORT_BE32(sample->sampleSize);
            loop = (struct AdpcmLoop*) (memBase + (uintptr_t) sample->loop);
            if (port_mark_swapped(sSwappedLoops, &sSwappedLoopCount, loop)) {
                loop->start = PORT_BE32(loop->start);
                loop->end = PORT_BE32(loop->end);
                loop->count = PORT_BE32(loop->count);
                if (loop->count != 0) {
                    for (k = 0; k < 16; k++) {
                        loop->state[k] = PORT_BE16(loop->state[k]);
                    }
                }
            }
            book = (struct AdpcmBook*) (memBase + (uintptr_t) sample->book);
            if (port_mark_swapped(sSwappedBooks, &sSwappedBookCount, book)) {
                book->order = PORT_BE32(book->order);
                book->npredictors = PORT_BE32(book->npredictors);
                n = 8 * book->order * book->npredictors;
                for (k = 0; k < n; k++) {
                    book->book[k] = PORT_BE16(book->book[k]);
                }
            }
        }
#endif
...
#define BASE_OFFSET(x, base) BASE_OFFSET_REAL(base, x)
...
#ifndef TARGET_N64
    mem->drums = (struct Drum**) PORT_BE32((uintptr_t) mem->drums);
#endif
    drums = mem->drums;
    numDrums2 = numDrums;
...
#ifndef TARGET_N64
            mem->drums[i] = (struct Drum*) PORT_BE32((uintptr_t) mem->drums[i]);
#endif
            patched = mem->drums[i];
            if (patched != NULL) {
...
#ifndef TARGET_N64
                    drum->envelope = (struct AdsrEnvelope*) PORT_BE32((uintptr_t) drum->envelope);
#endif
    ...
    if (numInstruments > 0) {
        ...
        do {
#ifndef TARGET_N64
            *itInstrs = (struct Instrument*) PORT_BE32((uintptr_t) *itInstrs);
#endif
            if (*itInstrs != NULL) {
...
#ifndef TARGET_N64
                    instrument->envelope = (struct AdsrEnvelope*) PORT_BE32((uintptr_t) instrument->envelope);
#endif
...
u8 get_missing_bank(u32 seqId, s32* nonNullCount, s32* nullCount) {
    ...
    *nullCount = 0;
    *nonNullCount = 0;
    offset = PORT_BE16(((u16*) gAlBankSets)[seqId]);
...
#define AUDIO_TRACE(...) port_log(__VA_ARGS__)
#else
#define AUDIO_TRACE(...)
#endif
/**
...
void audio_init(void) {
    ...
    u32 buf[2];
#else
    u32 buf[4]; // the 0x10 byte header DMAs below overflow buf[2]
#endif
...
#endif
    AUDIO_TRACE("audio_init: port_eu_init\n");
    port_eu_init();
    if (k) {} // fake
    ...
    AUDIO_TRACE("audio_init: pools (%d)\n", (int) gAudioInitPoolSize);
    ...
    AUDIO_TRACE("audio_init: reset step (init pool %p cur %p size %d)\n", gAudioInitPool.start, gAudioInitPool.cur, (int) gAudioInitPool.size);
    audio_shut_down_and_reset_step();
    AUDIO_TRACE("audio_init: after reset (init pool %p cur %p size %d, status %d)\n", gAudioInitPool.start, gAudioInitPool.cur, (int) gAudioInitPool.size, gAudioResetStatus);
    AUDIO_TRACE("audio_init: seq header @%p\n", &_sequencesSegmentRomStart);
    ...
    gSequenceCount = PORT_BE16(gSeqFileHeader->seqCount);
    AUDIO_TRACE("audio_init: seqCount %d rev %d\n", gSequenceCount, PORT_BE16(gSeqFileHeader->revision));
    size = gSequenceCount * sizeof(ALSeqData) + 4;
    size = ALIGN16(size);
    gSeqFileHeader = soundAlloc(&gAudioInitPool, size);
    AUDIO_TRACE("audio_init: seq table alloc %d -> %p (heap %p size %d)\n", size, gSeqFileHeader, gAudioHeap, (int) gAudioHeapSize);
    audio_dma_copy_immediate(data, gSeqFileHeader, size);
    AUDIO_TRACE("audio_init: relocating seq table\n");
    func_800BB43C(gSeqFileHeader, data);

    AUDIO_TRACE("audio_init: %d sequences, ctl @%p\n", gSequenceCount, &_audio_banksSegmentRomStart);
    ...
    ctlSeqCount = PORT_BE16(gAlCtlHeader->seqCount);
    ...
    for (i = 0; i < ctlSeqCount; i++) {
        ...
        gCtlEntries[i].numInstruments = PORT_BE32(buf[0]);
        gCtlEntries[i].numDrums = PORT_BE32(buf[1]);
    }

    AUDIO_TRACE("audio_init: %d banks, tbl @%p\n", ctlSeqCount, &_audio_tablesSegmentRomStart);
    ...
    size = PORT_BE16(gAlTbl->seqCount) * sizeof(ALSeqData) + 4;
    size = ALIGN16(size);
    gAlTbl = soundAlloc(&gAudioInitPool, size);
    AUDIO_TRACE("audio_init: tbl alloc %d -> %p\n", size, gAlTbl);
    audio_dma_copy_immediate(data, gAlTbl, size);
    AUDIO_TRACE("audio_init: tbl copied\n");
    func_800BB43C(gAlTbl, data);

    AUDIO_TRACE("audio_init: tbl ok, bank sets @%p\n", &_instrument_setsSegmentRomStart);
    ...
    AUDIO_TRACE("audio_init: init_sequence_players\n");
    init_sequence_players();
    gAudioLoadLock = 0x76557364;
    AUDIO_TRACE("audio_init: done\n");
}
