...
struct SPTask* create_next_audio_frame_task(void) {
    ...
    if (gAudioResetStatus != 0) {
         ...
         * sequence start together; the sequence-start command must stay queued
         * in D_800EA3AC until the reset's reload (which reallocates the note
         * pools and clears bank/seq load status) is done, or the just-started
         * sequence gets wiped and the music dies after ~0.25s. */
