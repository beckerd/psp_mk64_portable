...
/* The audio banks/tables are raw big-endian ROM data.  Ported targets are
 * little-endian, so multi-byte fields are swapped when the data is patched. */
#ifdef TARGET_N64
#define PORT_BE16(x) (x)
#define PORT_BE32(x) (x)
#else
#define PORT_BE16(x) ((u16) __builtin_bswap16((u16) (x)))
#define PORT_BE32(x) ((u32) __builtin_bswap32((u32) (x)))
#endif
...
struct EuAudioCmd {
...
#else
        /* The command word is built big-endian (op in the high byte); on a
         * little-endian target the bytes land in the opposite order. */
        struct {
            u8 arg3;
            u8 arg2;
            u8 bankId;
    ...
    union {
        ...
        s8 as_s8;
#else
        struct { u8 pad0[3]; u8 as_u8; };
        struct { u8 pad1[3]; s8 as_s8; };
#endif
    } u2;
};
