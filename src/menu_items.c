...
void func_80091B78(void) {
    ...
    PORT_LOG("  menu selection %d\n", gMenuSelection);
    if (gMenuSelection == LOGO_INTRO_MENU) {
        gNextFreeMemoryAddress = gFreeMemoryResetAnchor;
#ifndef TARGET_N64
        port_set_segment_table(6, &gPortStartupLogoSegTable);
    ...
    PORT_LOG("  setup_menus\n");
    setup_menus();
    PORT_LOG("  func_800C97C4\n");

    for (i = 0; i < 4; i++) {
        func_800C97C4((u8) i);
    }
    PORT_LOG("  func_800C9D0C\n");

    for (i = 1; i < 8; i++) {
        func_800C9D0C((u8) i);
    }
    PORT_LOG("  set_sound_mode\n");

    set_sound_mode();
    osViSetSpecialFeatures(OS_VI_DITHER_FILTER_ON);
    PORT_LOG("  func_80091B78 done\n");
}
...
void dma_copy_mio0_segment(u64* data, size_t nbytes, void* vaddr) {
    ...
    {
        static int n;
        const u8* src = ROM_SEG_PTR(_textures_0aSegmentRomStart, data);
        if (n++ < 40) {
            PORT_LOG("mio0 dma: data %p src %p nbytes %u magic %02X%02X%02X%02X -> %p\n", data, src, (unsigned) nbytes,
                     src[0], src[1], src[2], src[3], vaddr);
        }
    }
#endif

    osInvalDCache(vaddr, nbytes);
    osPiStartDma(&mb, OS_MESG_PRI_NORMAL, OS_READ, (uintptr_t) ROM_SEG_PTR(_textures_0aSegmentRomStart, data),
...
void dma_tkmk00_textures(u64* data, size_t nbytes, void* vaddr) {
    ...
    osInvalDCache(vaddr, nbytes);
    osPiStartDma(&mb, OS_MESG_PRI_NORMAL, OS_READ, (uintptr_t) ROM_SEG_PTR(_textures_0bSegmentRomStart, data),
                 vaddr, nbytes, &gDmaMesgQueue);
    osRecvMesg(&gDmaMesgQueue, &msg, OS_MESG_BLOCK);
}
...
#ifdef PORT_INPUT_SCRIPT
            PORT_LOG("menu img: data %p type %d %ux%u line %d -> buf %p idx 0x%X (entry %d)\n", texAddr->textureData,
                     texAddr->type, texAddr->width, texAddr->height, __LINE__, gMenuTextureBuffer, sMenuTextureBufferIndex,
                     sMenuTextureEntries);
#endif
...
void func_80099AEC(void) {
    ...
    osPiStartDma(&mb, 0, 0, (uintptr_t) ROM_SEG_PTR(_textures_0aSegmentRomStart, texPtr->textureData),
...
void func_80099EC4(void) {
    ...
    osPiStartDma(&sp68, 0, 0, (uintptr_t) ROM_SEG_PTR(_textures_0aSegmentRomStart, temp_s2->textureData),
...
void func_8009A9FC(s32 arg0, s32 arg1, u32 arg2, s32 arg3) {
    ...
    for (var_t1 = 0; (u32) var_t1 < arg2; var_t1++) {
        ...
        BE16_STORE(color1, newblue + newgreen + newred + alpha);
        color1++;
    }
}
...
void func_8009AB7C(s32 arg0) {
    ...
    for (var_v1 = 0; var_v1 < 0x4B000; var_v1++) {
        red = ((BE16_LOAD(color) & 0xF800) >> 0xB) * 0x4D;
        green = ((BE16_LOAD(color) & 0x7C0) >> 6) * 0x96;
        blue = ((BE16_LOAD(color) & 0x3E) >> 1) * 0x1D;
        alpha = BE16_LOAD(color) & 0x1;
        temp_t9 = red + green + blue;
        temp_t9 >>= 8;
        newred = temp_t9 << 0xB;
        newgreen = temp_t9 << 6;
        newblue = temp_t9 << 1;
        BE16_STORE(color, newblue + newgreen + newred + alpha);
        color++;
    }
}
...
void func_8009AD78(s32 arg0, s32 arg1) {
    ...
    for (var_v1 = 0; var_v1 != size; var_v1++) {
        ...
        BE16_STORE(color, (temp_t9 << 1) + (temp_t9 << 6) + (temp_t9 << 0xB) + alpha);
        color++;
    }
}
...
void convert_img_to_greyscale(s32 arg0, u32 arg1) {
    ...
    for (i = 0; i < (u32) size; i++) {
        red = ((BE16_LOAD(color) & 0xF800) >> 0xB) * 0x55;
        green = ((BE16_LOAD(color) & 0x7C0) >> 6) * 0x4B;
        blue = ((BE16_LOAD(color) & 0x3E) >> 1) * 0x5F;
        alpha = BE16_LOAD(color) & 0x1;
        temp_t9 = red + green + blue;
        temp_t9 /= 256;
...
void adjust_img_colour(s32 arg0, s32 arg1, s32 arg2, s32 arg3, s32 arg4) {
    ...
    for (var_v1 = 0; var_v1 != arg1; var_v1++) {
        ...
        BE16_STORE(color, newred + newgreen + newblue + alpha);
        color++;
    }
}
